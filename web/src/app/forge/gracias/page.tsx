  ...
  return (
    <main className="bg-white min-h-screen">
      ...
      <section className="relative overflow-hidden">
        <div aria-hidden className="pointer-events-none absolute inset-0 bg-[radial-gradient(ellipse_at_top,rgba(22,163,74,0.06),transparent_55%)]" />
        <div className="relative max-w-5xl mx-auto px-6 md:px-10 py-16 md:py-24 text-center">
          ...
          <h1 className="font-display text-5xl md:text-7xl uppercase text-neutral-900 leading-[0.9] tracking-tight mb-4">
            ¡Gracias {name}!
          </h1>
          <p className="text-xl md:text-2xl text-brand-green/90 font-medium mb-3">
            Content Forge ya es tuyo.
          </p>
          <p className="text-base md:text-lg text-neutral-600 max-w-2xl mx-auto leading-relaxed mb-10">
            También te enviamos el resumen completo a tu correo. Si no llega en 2 minutos, revisa tu carpeta de spam o escríbenos a <a href="mailto:founder@kreoon.com" className="text-brand-green hover:underline">founder@kreoon.com</a>.
          </p>
          ...
          <div className="flex flex-col sm:flex-row gap-3 justify-center max-w-2xl mx-auto">
            <Link
              ...
              className="flex-1 inline-flex items-center justify-center gap-2 bg-brand-green hover:bg-brand-green-hover text-white font-bold px-6 py-4 rounded-xl text-base transition-colors"
            >
              Abrir en GitHub →
            </Link>
            <Link
              ...
              target="_blank"
              rel="noopener noreferrer"
              className="flex-1 inline-flex items-center justify-center gap-2 text-brand-green hover:text-neutral-900 font-semibold px-6 py-4 rounded-xl text-base border border-brand-green/40 hover:border-brand-green transition-colors"
            >
              Guía de instalación
            </Link>
          </div>
      ...
      {/* BONUS: Priority call */}
      {hasPriority ? (
        <section className="py-12 border-y border-brand-green/30 bg-gradient-to-br from-brand-green/[0.06] via-transparent to-transparent">
          <div className="max-w-4xl mx-auto px-6 md:px-10 text-center">
            <span className="inline-flex items-center px-3 py-1 rounded-full text-[10px] font-bold tracking-widest uppercase bg-brand-green/20 text-brand-green border border-brand-green mb-4">
              ⭐ BONUS DESBLOQUEADO
            </span>
            ...
            <Link
              ...
              target="_blank"
              rel="noopener noreferrer"
              className="inline-flex items-center gap-2 bg-brand-green hover:bg-brand-green-hover text-white font-bold px-8 py-4 rounded-xl text-base transition-colors"
            >
      ...
      <section className="bg-neutral-50 py-20 md:py-24 border-t border-neutral-100">
        <div className="max-w-5xl mx-auto px-6 md:px-10">
          <div className="text-center mb-12">
            <span className="text-[10px] uppercase tracking-widest font-bold text-brand-green">
              Cómo empezar
            </span>
            <h2 className="font-display text-4xl md:text-5xl uppercase text-neutral-900 leading-tight tracking-tight mt-3">
              4 pasos · 20 minutos
            </h2>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {STEPS.map((s, i) => (
              <article key={s.title} className="flex gap-4 p-5 rounded-2xl border border-neutral-200 bg-white">
                <div className="flex-shrink-0 w-10 h-10 rounded-full bg-brand-green/20 text-brand-green font-bold flex items-center justify-center text-base">
                  {i + 1}
                </div>
                <div className="flex-1">
                  <h3 className="font-semibold text-neutral-900 text-base mb-1">{s.title}</h3>
                  <p className="text-sm text-neutral-600 leading-relaxed mb-2">{s.body}</p>
                  {s.code ? (
                    <code className="block font-mono text-xs text-brand-green bg-neutral-100 border border-neutral-200 rounded px-3 py-2">
                      {s.code}
                    </code>
                  ) : null}
                </div>
      ...
      <section className="bg-neutral-50 py-16 border-t border-neutral-100">
        <div className="max-w-3xl mx-auto px-6 md:px-10 text-center">
          <h3 className="font-display text-3xl md:text-4xl uppercase text-neutral-900 leading-tight mb-3">
            Síguenos y aprende más
          </h3>
          <p className="text-sm text-neutral-600 mb-8 max-w-lg mx-auto">
