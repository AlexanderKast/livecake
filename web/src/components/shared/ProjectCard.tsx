...
const STATUS_CONFIG: Record<ProjectStatus, { label: string; bg: string; fg: string; border: string }> = {
  ...
  new: { label: "NUEVO", bg: "bg-brand-green/15", fg: "text-brand-green", border: "border-brand-green/60" },
  ...
  return (
    <article
      className={`group flex flex-col gap-4 p-6 rounded-2xl border transition-all ${
        props.featured
          ? "border-brand-green/60 bg-gradient-to-br from-brand-green/10 via-transparent to-transparent"
          : "border-white/10 bg-black/40 hover:border-brand-green/40"
      }`}
    >
      {/* Status + featured */}
      <div className="flex items-center justify-between gap-2">
        ...
        {props.featured ? (
          <span className="text-[10px] font-bold tracking-widest uppercase text-brand-green">
            ★ DESTACADO
          </span>
        ) : null}
      </div>
      ...
      <div>
        ...
        <p className="text-sm text-brand-green/90 font-medium">{props.tagline}</p>
      </div>
          ...
          <>
            {props.leadFormUrl ? (
              <Link
                ...
                className="inline-flex items-center justify-center gap-2 bg-brand-green hover:bg-brand-green-hover text-brand-black font-bold px-5 py-3 rounded-xl text-sm transition-colors"
              >
                Obtener gratis →
              </Link>
            ) : props.repoUrl ? (
            ...
            {props.guideUrl ? (
              <Link
                ...
                target="_blank"
                rel="noopener noreferrer"
                className="inline-flex items-center justify-center gap-2 text-brand-green hover:text-brand-green-hover font-semibold px-5 py-2 rounded-xl text-sm border border-brand-green/30 hover:border-brand-green/60 transition-colors"
              >
                Guía de instalación
              </Link>
            ) : null}
